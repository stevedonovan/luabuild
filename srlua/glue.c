...
static long copy(const char* name, FILE* out, const char* outname)
{
 ...
 for (;;)
 {
  ...
  offs = 0;
  if (first) {
    if (b[0] == '#' && b[1] == '!' && b[2] == '/') { // get rid of shebang!
        offs += 3;
        while (b[offs] != '\n')
            ++offs;
        ++offs; // past line feed!
        size -= offs; // adjust size of written file
    }
    first = 0;
  }
  if (fwrite(&b[offs],n-offs,1,out)!=1) cannot("write",outname);
 }
 fclose(f);
 return size;
}
...
int main(int argc, char* argv[])
{
 ...
 {
   char path[256];
   sprintf(path,"chmod +x %s",argv[3]);
   if (system(path)!=0) cannot("chmod",argv[3]);
 }
#endif
 return 0;
}
